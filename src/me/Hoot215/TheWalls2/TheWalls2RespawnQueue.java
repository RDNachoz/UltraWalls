...
public class TheWalls2RespawnQueue{
	...
	public Location getLastPlayerLocation(String playerName){
		...
		for(String s:respawnQueue){
			...
			if (a[0].equals(playerName)){
				loc=new Location(plugin.getServer().getWorld(a[1]),Double.parseDouble(a[2]),Double.parseDouble(a[3]),Double.parseDouble(a[4]));
			}
		}
	...
	public void addPlayer(String playerName, Location loc){
		respawnQueue.add(playerName+":"+loc.getWorld().getName()+":"+loc.getBlockX()+":"+loc.getBlockY()+":"+loc.getBlockZ());
	}

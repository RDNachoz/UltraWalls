    ...
    public void onPlayerDeath (PlayerDeathEvent event)
      {
        ...
        if (gameList.isInGame(playerName))
          {
          plugin.getGameList();gameList.notifyAll(
                ChatColor.YELLOW.toString() + playerName + ChatColor.RED.toString()
        ...
        if (respawnQueue.isInRespawnQueue(playerName))
          {
            ...
            plugin.getServer().getScheduler().runTaskLater(plugin,new Runnable(){
              public void run(){
                ...
                if (fplayer!=null)plugin.getServer().dispatchCommand(fplayer,"warp Standing_Stones");
              }
            },5L);}
